...
local Item = require("items.item")  -- Make sure to require your Item module
local RustyKnife = {}
...
function RustyKnife:use(playerDirection)  
    if self.isCoolingDown or self.swingState ~= "stopped" then
        return
    end

    print("Swinged Rusty Knife!")
    self.swooshSound:play() 

    self.swingState = "forward"  
    self.timer = 0               
    self.direction = playerDirection  -- Set the direction based on the player's current direction
end
...
function RustyKnife:update(dt)
    ...
    -- Calculate rotation based on the current swing angle and direction
    self.rotation = self.swingAngle * (self.direction == "right" and 1 or -1)
end

function RustyKnife:draw(playerX, playerY)
    ...
    if texture then
        ...
        print("Error: Texture for Rusty Knife not found!")
    end
end
