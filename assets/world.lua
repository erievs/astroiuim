...
function World.draw()
    ...
    local hoveredX = math.floor(mouseX / World.tileSize) + 1
    local hoveredY = math.floor(mouseY / World.tileSize) + 1
    ...
    if closestBlock then
        ...
        local tier = closestBlock:getTier()
        local blockType = closestBlock.type or "unknown"
        love.graphics.print(string.format("Type: %s, Tier: %d", blockType, tier), mouseX + 10, mouseY + 10)
    end
end
